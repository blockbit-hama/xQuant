...
use crate::indicators::{Indicator, IndicatorResult, moving_averages::MovingAverageCrossover};
...
impl MACrossoverBot {
  pub fn new(symbol: String, config: TradingBotConfig) -> Result<Self, TradingError> {
    ...
    let ma_type = config.get_string("ma_type").unwrap_or_else(|_| "EMA".to_string());
    
    // MA 크로스오버 지표 생성
    let ma_crossover = match ma_type.as_str() {
      "SMA" => MovingAverageCrossover::with_sma(fast_period, slow_period),
      _ => MovingAverageCrossover::with_ema(fast_period, slow_period),
    };
...
impl TradingBot for MACrossoverBot {
  fn update(&mut self, market_data: &MarketData) -> Result<(), TradingError> {
    ...
    if self.ma_crossover.is_ready() {
      let result = self.ma_crossover.calculate()?;
  ...
  fn update_config(&mut self, config: TradingBotConfig) -> Result<(), TradingError> {
    ...
    let ma_type = self.config.get_string("ma_type").unwrap_or_else(|_| "EMA".to_string());
    
    // MA 크로스오버 지표 재생성
    self.ma_crossover = match ma_type.as_str() {
      "SMA" => MovingAverageCrossover::with_sma(fast_period, slow_period),
      _ => MovingAverageCrossover::with_ema(fast_period, slow_period),
    };
